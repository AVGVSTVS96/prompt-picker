...
const codex: Fake[] = [
  ...
  { text: "typecheck fails only under bun 1.3 with a phantom circular import — bisect and fix", ago: 2 * HOUR + 10 * MIN, project: "patch-md", model: "gpt-5.5" },
  { text: "vendor the wasm build of the tokenizer and wire it behind the existing interface", ago: 11 * HOUR, project: "notes-search", model: "gpt-5.5" },
  ...
  { text: "port the retry queue from polling to LISTEN/NOTIFY and delete the cron fallback", ago: 7 * HOUR, project: "atlas-api", model: "gpt-5.5" },
  ...
  { text: "the tokenizer cache grows unbounded in long sessions — add an LRU with a 512 entry cap and tests", ago: 4 * DAY, project: "patch-md", model: "gpt-5.5" },
];
...
const pi: Fake[] = [
  ...
  { text: "audit the keyboard navigation across all modals against the WAI-ARIA dialog pattern", ago: 1 * DAY + 10 * HOUR, project: "design-system", model: "claude-opus-4-7" },
  ...
  { text: "explain the tradeoffs of storing embeddings in sqlite vs a dedicated vector store for 100k rows", ago: 2 * DAY + 2 * HOUR, project: "notes-search", model: "gpt-5.5" },
